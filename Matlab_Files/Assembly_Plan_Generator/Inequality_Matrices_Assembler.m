function [A,b] = Inequality_Matrices_Assembler(F,force_f,model,T)
%Create A and b matrices (inequalities) of the linear programing problem 
%(A.x <= b).
%
...
%        b: number_of_knobs by 1 matrix.
    ...
    force = size(F,1);
    A = spalloc(n_knob, 3*force+1,force+n_knob); % Each line represents one knob, end each set of 3 columns is one force (components in (X,Y,Z).

    for i = 1:force_f % For each friction force defined
