%% Stability_Judge_pushing_180907_bot.m 組立終了時
% ブロックモデルの力学解析
%%constant number
...
%% 押し込むブロックを除いたモデルデータ
model_pushing = model(N, :);
model(N, :) = []
knobs = knob(model);
z_max_push = lines(model);
model_push = model;
for i = N-1 : -1 : 1
   if( model(i, 4) == z_max)
      model_push(i, :) = [];
...
model_push = [model_push; model_pushing]
...
% ff_zmaxの向きを修正
F( (force_f - ff_zmax(1) + 1):force_f, 7:6:13) = -F((force_f - ff_zmax(1) + 1):force_f, 7:6:13);
...
for n = 1 : force_f;
    if(check ~= 0)
        ...
        for m = 1 : 23
            ...
            if((F_f(n+m, 2)~=block_1) && (F_f(n+m, 8)~=block_2))
                break;
            else
                n_block = n_block + 1;
            end
...
%% Linear equalities
Aeq = zeros(6*(N-1), 3*force+1);
beq = zeros(6*(N-1), 1);
% 押し込むブロックの力・モーメントのつり合いは考慮しない
for n = 1 : N-1
...
%% (押し込むブロックの摩擦力の合力) = T
force_f = force_f + ff_zmax(1)
l_block = F(force_f, 8);  l_knob = F(force_f, 9);
m_zmax = 1;
beq( (6*(N-1)+1) ) = T;
for i = force_f : -1 : 1
    if(F(i, 2) ~= N)
    ...
    if( (F(i, 8) == l_block) && (F(i, 9) == l_knob))
        Aeq(6*(N-1) + m_zmax, 3*i) = 1;
        %check = 'A'
    else
        l_block = F(i, 8);  l_knob = F(i, 9);
        m_zmax = m_zmax + 1;
        Aeq(6*(N-1) + m_zmax, 3*i) = 1;
        beq( (6*(N-1)+m_zmax) ) = T;
    end
