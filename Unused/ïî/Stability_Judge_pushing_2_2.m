...
%%
%%押し込み力の働くブロックの摩擦力の向きを調整
force_f_push = 0;
for i = 1 : force_f
   if(F_f(i, 2)==N)
       F_f(i, 6:5:11) = [1, -1];
       force_f_push = force_f_push + 1;
   end
end
%% Linear inequalities
%Cmaxを評価するのは凸の数．ただし，押し込むブロックは除く
%
model_push = model(1:(N - 1), 1:5);
knobs_push = knob(model_push);
%
SIZE = size(knobs_push);  n_knob = SIZE(1);
A = zeros(n_knob, 3*force+1);
b  = ones(n_knob, 1) * T;
count = 1;  check = 0;
for n = 1 : force_f-force_f_push;
    if(check ~= 0)
        ...
        if(n_block == 4) %凸 1
            ...
            A(count, 3*n : 3 : 3*n+9) = 1;
            check = 4;  count = count + 1;
        elseif(n_block == 3) %凸 1
            A(count, 3*n : 3 : 3*n+6) = 1;
            check = 3;  count = count + 1;
        elseif(n_block == 6) %凸 2
            A(count, 3*n : 3 : 3*n+6) = 1;
            A(count+1, 3*n+9 : 3 : 3*n+15) = 1;
            check = 6;  count = count + 2;
        elseif(n_block == 9) %凸 3
            A(count, 3*n : 3 : 3*n+6) = 1;
            A(count+1, 3*n+9 : 3 : 3*n+15) = 1;
            A(count+2, 3*n+18 : 3 : 3*n+24) = 1;
            check = 9;  count = count + 3;
        elseif(n_block == 12) %凸 4
            A(count, 3*n : 3 : 3*n+6) = 1;
            A(count+1, 3*n+9 : 3 : 3*n+15) = 1;
            A(count+2, 3*n+18 : 3 : 3*n+24) = 1;
            A(count+3, 3*n+27 : 3 : 3*n+33) = 1;
            check = 12;  count = count + 4;
        end
    end
    check = check - 1;
end
...
%押し込み力を追加
beq
join_push = join(knobs, z_max);
SIZE = size(join_push);
beq(6*N-3) = beq(6*N-3) + T*SIZE(1)
%% Solve proble
f = zeros(3*force+1, 1);
f((3*force + 1), 1) = 1;
