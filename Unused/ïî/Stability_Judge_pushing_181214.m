%% Stability_Judg_pushing_181214.m
% ブロックモデルの力学解析(ブロック挿入の瞬間)  % Fn' 上限値設定
%% constant number
...
%% 挿入中の力学解析 % ブロックモデル最後のブロックを挿入
SIZE = size(model);  N = SIZE(1);
% 押し込み力の加わるブロック番号と加わるモーメント
...
for i = 1 : force_f
   if( ((F(i, 4) == -0.75)||(F(i, 4) == 0.75))&&((F(i, 5) == -2)||(F(i, 5) == 2)) )
       if( (F(i, 6) == -1.5) || (F(i, 6) == 1.5) )
          %F(i, 6) = F(i, 6) + 0.5;
          lb(3*i-2:3*i) = zeros(3, 1);
          ub(3*i-2:3*i) = zeros(3, 1);
       end
   end
end
LB = [lb(1:3:3*force_f-2), lb(2:3:3*force_f-1), lb(3:3:3*force_f)];
...
for n = 1 : N_push
    pbn = b_push(n, 1);   % 押し込み力の加わるブロック番号
