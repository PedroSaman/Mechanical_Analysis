...
for n = 1 : z_max %for each layer
   if(n == 1) % if it is the first layer
       ...
       if(adjoin_i == -1) %No adjacent blocks 
           F_nx = -1;  F_ny = -1;
           force_nx = 0;  force_ny = 0;
       else %There are adjacent blocks 
           Fnx_i = Fnx(adjoin_i); %Normal force in the x axis
           Fny_i = Fny(adjoin_i); %Normal force in the y axis
           if(Fnx_i(1) ~= -1) %There is no normal force in X axis
               F_nx = Fnx_i;
               force_nx = -1;
           else
               force_nx = 0;
           end
           if(Fny_i(1) ~= -1) %There is no normal force in Y axis
               F_ny = Fny_i;
               force_ny = -1;
           else
               force_ny = 0;
   ...
   else %if the layer is >= 2
       %Convex knob_i, adjacent block adjoin_i, connection convex join_i 
       knob_i = knob_line(model, n); %knobs in this layer information
       adjoin_i = adjoin(knob_i); %adjoint blocks information
...
%% Upper and lower limit confirmation (x,y,z)
