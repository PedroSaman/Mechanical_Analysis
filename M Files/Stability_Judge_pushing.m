...
b_push = pushing(model, N); %(Block_No., Fx, Fy, Fz, Mx, My, Mz)
N = N - 1;
model = model(1:(N - 1), :); % Take off the last block from the model
...
SIZE = size(b_push);
N_push = SIZE(1); %Number of blocks to which pushing force is applied
for n = 1 : N_push
    pbn = b_push(n, 1); %The number of the block to which the pushing force is applied 
    b_push(n, 4) = b_push(n, 4) + beq(6*pbn-3); %Pushing force + gravity 
