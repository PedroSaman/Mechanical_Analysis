...
%% ブロックモデルデータ最終行のブロックを挿入
SIZE = size(model);  N = SIZE(1);
% 押し込み力の加わるブロックの番号，押し込まれることでブロックに加わる力とモーメントを調べる
%   → b_push = (Block_No., Fx, Fy, Fz, Mx, My, Mz)
b_push = pushing(model, N);
%挿入するブロックをブロックモデルデータから削除
model = model(1:(N - 1), :);
...
SIZE = size(b_push);
N_push = SIZE(1);   % 押し込み力の加わるブロックの数
for n = 1 : N_push
    pbn = b_push(n, 1);   % 押し込み力の加わるブロックの番号
    b_push(n, 4) = b_push(n, 4) + beq(6*pbn-3);   % 押し込み力 + 重力
...
f = zeros(3*force+1, 1);
f((3*force + 1), 1) = 1;
[x,fval,exitflag,output] = linprog(f,A,b,Aeq,beq,lb,ub)  %線形計画問題
XX =[x(1:3:3*force-2), x(2:3:3*force-1), x(3:3:3*force)]  %ブロックモデルに働く力
CM = x(3*force+1) %キャパシティCM
time = toc  %計測時間表示