...
private:
  ...
  std::string descend_to_pickPlan(environment_interface::msg::Block block, std::string robot_name, std::string planner_id);
  ...
  std::string descend_to_placePlan(environment_interface::msg::Block block, std::string robot_name, std::string planner_id);
...
private:
  ...
  void execute(const std::shared_ptr<GoalHandleTaskCreator> goal_handle)
  {
    ...
    result->task_resposnse = mtc_node->doTask(block, operation_id, robot_name, planner_id);
...
{ 
  ...
  case DESCEND_TO_PICK:
    plan_outout = descend_to_pickPlan(block, robot_name, planner_id);
  ...
  case DESCEND_TO_PLACE:
    plan_outout = descend_to_placePlan(block, robot_name, planner_id);
...
std::string MTCTaskNode::descend_to_pickPlan(environment_interface::msg::Block block, std::string robot_name, std::string planner_id)
{
  const auto& arm_group_name = robot_name + "_arm";
  const auto& hand_frame = "gripper_tip";
...
std::string MTCTaskNode::descend_to_placePlan(environment_interface::msg::Block block, std::string robot_name, std::string planner_id)
{
  const auto& arm_group_name = robot_name + "_arm";
  const auto& hand_frame = "gripper_tip";
